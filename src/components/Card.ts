...
export class Card<T extends IProductItem | {}> extends Component<T | IProductItem> {
	...
	constructor(
		...
		if (actions?.onClick) {
            if (this._buttonModal) { // Ставим событие на кнопку купить в превью заказа
                this._buttonModal.textContent = 'Купить';
                this._buttonModal.addEventListener('click', actions.onClick);
            } else { // Ставим событие на заказ в гадерее 
    ...
    set category(value:string) {
        ...
        this.toggleClass(this._category, bem(this.blockName, 'category', category[value]).name, true);
    }
